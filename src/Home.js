...
export default function Home() {
    ...
    // this loads the pets from database when the site loads
    useEffect(() => {
    ...
    // When we put something in the database, this doesn't change the state, we need to do that too.
    const handleAddPet = (pet) => {
        const updatedPets = createPet(pet); // save the pet to database, and return the updated list
        setPets(updatedPets); // update state to trigger rerender
    };

    // Same as above, but for deleting a pet
    const handleDeletePet = (id) => {
        const updatedPets = deletePet(id); //delete from database
